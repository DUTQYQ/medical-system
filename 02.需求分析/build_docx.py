"""从 Markdown 生成适合评审和打印的 Word 交付件（02.需求分析 · 原型设计说明书 v2.0）。

与 03.概要设计/build_docx.py 同源：统一 A4 页面、标题字体、表格样式、页码与图片宽度。
"""
...
def column_weights(headers: list[str]) -> list[float]:
    n = len(headers)
    joined = "|".join(headers)
    if n == 2 and headers[0] == "项":
        return [0.26, 0.74]
    if n == 2:
        return [0.34, 0.66]
    if n == 4 and "版本" in headers:
        return [0.12, 0.14, 0.12, 0.62]
    if n == 4 and "编号" in headers[0]:
        return [0.22, 0.30, 0.48]
    if n == 4:
        return [0.16, 0.28, 0.26, 0.30]
    if n == 5 and "覆盖状态" in joined:
        return [0.06, 0.19, 0.19, 0.14, 0.42]
    if n == 5 and "分组" in joined:
        return [0.30, 0.14, 0.14, 0.14, 0.28]
    if n == 5:
        return [0.10, 0.24, 0.28, 0.16, 0.22]
    return [1.0 / n] * n
...
def main() -> None:
    ...
    for source_name in SOURCES:
        ...
        with tempfile.TemporaryDirectory(prefix="kangyang_proto_") as temp_dir:
            ...
            subprocess.run(
                [
                    ...
                    str(raw),
                    "--dpi=300",
                    f"--resource-path={BASE};{BASE / '界面原型'}",
                ],
                cwd=BASE,
                check=True,
            )
